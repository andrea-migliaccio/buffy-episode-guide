# Buffy the Vampire Slayer – Episode Importance Guide

Welcome to a comprehensive, continuity-focused guide to **Buffy the Vampire Slayer**.  
This project is designed to help both new viewers and longtime fans navigate the series with clarity — especially if you prefer a *selective watch* rather than viewing all 144 episodes.

Each season includes:
- A short overview of the main story arcs  
- A full episode list with importance classification  
- Concise episode summaries  
- **Key Story Notes** explaining continuity, lore, character introductions, and callbacks  

The goal is to provide an easy way to understand *which episodes truly matter* and *why*, while still capturing important story seeds hidden inside otherwise skippable episodes.

---

## 🧭 How to Use This Guide

Every season follows the same table structure:

| Code | Title | Importance | Description | Key Story Notes |
|------|--------|------------|-------------|-----------------|
| SxxEyy | Episode title | FONDAMENTAL / IMPORTANT / DISPENSABLE / FILLER | Short plot summary | Continuity, lore, character intros, future callbacks |

### **Importance Levels**

- **FONDAMENTAL** — Essential to the core storyline or major Buffyverse continuity  
- **IMPORTANT** — Significant character development or mythology with future impact  
- **DISPENSABLE** — Skippable, but may contain useful minor threads  
- **FILLER** — Largely standalone with little to no impact on continuity  

### **Key Story Notes**

This column highlights:
- crucial character introductions  
- lore and mythology seeds  
- plot points referenced again in later seasons  
- relationship developments that shape future arcs  
- ties to the spin-off *Angel* when relevant  

This makes it possible to **skip filler episodes without losing context**.

---

## 📁 Seasons Overview

Use the links below to explore each season’s guide:

- [Season 1](./season1.md)  
- [Season 2](./season2.md)  
- [Season 3](./season3.md)  
- [Season 4](./season4.md)  
- [Season 5](./season5.md)  
- [Season 6](./season6.md)  
- [Season 7](./season7.md)  

(A similar guide for *Angel* will be added in the future.)

---

## ⚠️ Spoiler Notice

Because this guide focuses on continuity, the **Key Story Notes** column may reference characters or events that appear later in the series.  
If you are a first-time viewer and wish to avoid spoilers, proceed carefully.

---

## 🤝 Community & Contributions

This project reflects a curated interpretation of the Buffyverse, but many elements —  
such as episode importance, continuity interpretations, and story relevance — are naturally **subjective**.

For this reason, **contributions from the community are warmly welcomed**.

The project is fully **open source**, and you can help improve it by:
- refining descriptions  
- adding or correcting continuity notes  
- discussing episode importance  
- improving structure and accuracy  

🔗 **Project repository:**  
https://github.com/andrea-migliaccio/buffy-episode-guide

🛠 **How to contribute:**  
See the guidelines here:  
https://github.com/andrea-migliaccio/buffy-episode-guide/blob/main/CONTRIBUTING.md

